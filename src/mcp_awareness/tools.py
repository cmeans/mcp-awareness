...
Tool functions are defined at module level (using helpers.store) so they can
be imported by tests.  register_tools() wires them into the FastMCP instance.
"""
...
def register_tools(mcp: FastMCP, store_ref: Any) -> None:
    """Register all tool handler functions on the given FastMCP instance."""
    _tool = mcp.tool()
    _tool(get_briefing)
    _tool(get_alerts)
    _tool(get_status)
    _tool(get_knowledge)
    _tool(get_suppressions)
    _tool(report_status)
    _tool(report_alert)
    _tool(learn_pattern)
    _tool(remember)
    ...
    _tool(get_stats)
    _tool(get_tags)
    _tool(suppress_alert)
    _tool(add_context)
    _tool(set_preference)
    _tool(delete_entry)
    _tool(restore_entry)
    _tool(get_deleted)
    _tool(acted_on)
    ...
    _tool(get_activity)
    _tool(remind)
    _tool(get_intentions)
    _tool(update_intention)
    _tool(semantic_search)
    _tool(backfill_embeddings)
    _tool(get_related)
