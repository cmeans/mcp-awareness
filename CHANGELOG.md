...
- **`.github/dependabot.yml` — weekly automated dependency updates** across four ecosystems: `pip` (runtime + dev deps from `pyproject.toml`, split into `python-production` and `python-development` groups so runtime bumps don't share PRs with dev-tool bumps), `github-actions` (all workflow step versions grouped), `docker` (Dockerfile base image), and `docker-compose` (pinned images in `docker-compose.yaml`, e.g. `pgvector/pgvector:pg17`; `:latest` tags are skipped by Dependabot). Schedule: Mondays 03:00 America/Chicago, grouped minor+patch version updates per ecosystem, open-PR limit 5. Security updates are *not* batched — Dependabot always opens an individual PR per CVE regardless of the groups above, so critical fixes land immediately. PRs are labeled `dependencies` and commit messages use the repo's `chore(deps)` prefix with scope.
