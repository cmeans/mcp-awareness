...
### Added
- **Language resolution helpers** — `src/mcp_awareness/language.py` — the resolution chain used at write time and query time for Layer 1 of hybrid retrieval (explicit → user preference → `lingua-py` auto-detection → `simple` fallback). Pure module, no schema or storage changes. ISO 639-1 ↔ Postgres `regconfig` mapping covers 28 built-in configurations plus 4 via the `pgroonga` extension (`japanese`, `chinese_simplified`, `korean`, `hebrew`). Lazy detector loading so callers that don't exercise detection don't pay the import cost. `lingua-language-detector>=2.0,<3.0` added as a runtime dependency. Refs [#238](https://github.com/cmeans/mcp-awareness/issues/238).
