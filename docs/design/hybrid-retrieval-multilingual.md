...
**Verification results — Steps 0 + 1 executed 2026-04-11:**

Both Step 0 (Substantive 3, gating) and Step 1 (Substantive 2, generated-column pattern) were executed against `groonga/pgroonga:latest-alpine-17` (PostgreSQL 17.9 on x86_64-pc-linux-musl, pgroonga 4.0.6). Empirical outcomes documented here so the wiring PR has concrete answers, not planning checkboxes.

*Step 0 outcome — pgroonga regconfig integration:* **NO. pgroonga 4.0.6 does not register any entries in `pg_ts_config`.** Baseline before `CREATE EXTENSION pgroonga` returned 29 rows (28 stock snowball regconfigs + `simple`). After `CREATE EXTENSION pgroonga`, the same query returned an *identical* 29 rows. None of the regconfig names the design assumed pgroonga would register (`japanese`, `chinese_simplified`, `chinese_traditional`, `korean`, `hebrew`) appear in `pg_ts_config` either before or after extension installation. `SELECT to_tsvector('japanese', '日本語のサンプルテキストです')` errors with `text search configuration "japanese" does not exist`, exactly the failure mode the design predicted.

The pgroonga extension itself is fully functional under its documented integration model: a control table created with `CREATE INDEX ... USING pgroonga (content)` and queried via the `&@` operator successfully indexes and retrieves both Japanese and Chinese text. The regconfig absence is the intentional design of pgroonga, not a packaging bug, not a missing build flag, not an Alpine-specific quirk. **The four pgroonga-listed entries that #246 round 5 removed preemptively from `ISO_639_1_TO_REGCONFIG` were correctly removed and cannot be added back under any pgroonga path.**

*Step 1 outcome — generated-column + per-row regconfig pattern on PG17.9:* **YES. Pattern works.** The exact pattern Layer 1 needs (`tsv tsvector GENERATED ALWAYS AS (to_tsvector(language, content)) STORED` where `language` is a per-row `regconfig` column) is accepted at `CREATE TABLE` time on PG17.9, populates the `tsv` column correctly on `INSERT` for each row's specific regconfig (verified for `english`/`spanish`/`french`/`simple` — each producing language-specific snowball stemming), regenerates the `tsv` column dynamically when the `language` column is updated on an existing row (an English-stemmed row updated to `language='spanish'` re-tokenizes through the Spanish stemmer, retaining English stop words like "the" and "over" that English's stemmer would have dropped), works with a standard `GIN` index on `tsv` (verified via `EXPLAIN ANALYZE` with `enable_seqscan=off` showing `Bitmap Index Scan on entries_test_tsv_gin` for a `@@` query), and fails at `INSERT` time with `ERROR: text search configuration "japanese" does not exist` when handed a regconfig name that doesn't exist in `pg_ts_config` — exactly the case Substantive 2's startup-cache validation is designed to catch and degrade gracefully via the unsupported-language fallback.

**The trigger-based fallback documented above is therefore not needed for the Layer 1 wiring PR.** It remains in this design doc as a documented escape hatch in case a future PG minor version regresses or a managed-Postgres provider rejects the generated-column pattern (Step 3, the RDS verification, is still open and could surface this).

*Step 1 partial — hybrid CTE plan with HNSW + GIN:* **Not exercised in this verification pass.** The `groonga/pgroonga:latest-alpine-17` image does not include `pgvector`, so the combined hybrid-retrieval CTE plan (HNSW vector branch + GIN lexical branch + RRF fusion) was not verified in this session. This is the one Step 1 checkbox that remains genuinely open. The wiring PR's Alembic migration will need to confirm the combined plan against a `pgvector/pgvector:pg17` image with the chosen non-Western FTS extension layered in (or no non-Western FTS extension if Phase 1.05 chooses option 3).

*Step 2 outcome — memory measurement (#248):* **Not run in this session.** Step 2 was originally planned to measure pgroonga's memory cost. With Step 0 returning a definitive "no regconfigs," the relevant memory measurement is now contingent on Phase 1.05's choice — measuring the memory cost of the chosen alternative (zhparser, branched-pgroonga path, or no non-Western extension at all in the deferral case). #248 should be reframed accordingly when Phase 1.05 closes; until then, the stock-snowball-config measurements (the 28 entries Layer 1 will definitely use) are still meaningful and could be run independently.

*Step 3 outcome — RDS / managed Postgres compatibility:* **Not run in this session.** Step 3 is unchanged in scope and remains open for the wiring PR (or for #248 if it gets reframed to cover stock-language scope).

*Phase 1.05 status after this verification:* **Now the load-bearing open decision.** Step 0's negative result rules out the original "install pgroonga, use the 4 entries" path that the design assumed. The three options remain (per-language parser extensions like zhparser, pgroonga with branched query path, defer non-Western from Layer 1) and one of them must be picked before the wiring PR's Alembic migration can be written for any non-Western language path. The wiring PR is *not* blocked on this for stock-language scope (the 28 snowball regconfigs work today and are the bulk of Layer 1's value); it is only blocked for non-Western scope.

*Verification artifacts:* preserved at `~/.local/state/mcp-awareness-pg-verification/` (baseline `pg_ts_config`, post-pgroonga `pg_ts_config`, Japanese error output, Japanese INSERT failure output). Container `pg-verify-249` was destroyed after capture; outputs are reproducible by re-running the steps above.
...
| External search index (Typesense / Meilisearch) | 0 Postgres extensions; new standalone search service | All CJK + Hebrew + many more, with `locale`-tagged fields | Replaces the entire Layer 1 hybrid CTE | Available for future reconsideration; empirically tested in a 20-test spike on 2026-04-11 |
...
- **Typesense 29.0**: 20-operation spike on 2026-04-11 confirmed multilingual lexical search (with `locale="ja"`/`"zh"`/`"ko"` field-level tokenization), built-in vector + lexical hybrid via `multi_search` with `vector_query`, multi-tenant filtering, tag intersection / NOT-tag, faceting, soft delete with sentinel pattern, upsert, and nested JSON. Critical caveat: requires per-language fields for non-Western languages — cannot have one universal `content` field that handles all languages with proper tokenization. Also lacks ACID transactions across documents and DB-enforced RLS (would need application-level reconciliation).
...
2. Re-evaluate the architecture options against the data of the day. Three were on the table when Phase 1.05 closed:
   ...
   - **External search index** (Typesense or Meilisearch — empirically verified to handle CJK with `locale`-tagged fields in a 20-test spike on 2026-04-11; trades the Postgres-only data layer for a two-system topology with sync; unblocks managed-Postgres compatibility)
